    ...
    class TrailStop {
      ...
      calculateBezierCurves() {
        ...
        for (let i = 0; i < this.bezierCurvePoints_.length; ++i) {
          ...
          const previousPoints = this.previousStop_.bezierCurvePoints_[i].map(
            (p) =>
              this.obj_.worldToLocal(
                this.previousStop_.obj_.localToWorld(p.clone())
              )
          )
          ...
          for (let i = 0; i < allCurvePoints.length; ++i) {
            ...
            for (let k = 0; k < allCurvePoints[i].length; ++k) {
              ...
              const geometry = new THREE.BufferGeometry().setFromPoints(
                curve.getPoints(
                  TrailStop.options.numberOfPointsOnCrossCurvePiece
                )
              )
      ...
      constructor(scene) {
        ...
        for (const particle of this.particles_) {
          ...
          particle.time += THREE.MathUtils.randFloatSpread(
            Trail.options.maxParticleSpawnDelay *
              Trail.options.numberOfParticles
          )
        }

        this.timeElapsed =
          this.stops_[Trail.options.indexOfCameraStop].startTime_
      }
